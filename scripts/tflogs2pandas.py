...
# Extraction function
def tflog2pandas(logdir_or_logfile: str, path: str) -> pd.DataFrame:
    ...
    try:
        ...
        for tag in tags:
            new_tag = path.replace(logdir_or_logfile, '').split('/')[1:-1]
            if tag not in new_tag:
                new_tag += [tag]
            event_list = event_acc.Scalars(tag)
            values = list(map(lambda x: x.value, event_list))
            step = list(map(lambda x: x.step, event_list))
            r = {"metric": ["/".join(new_tag)] * len(step), "value": values, "step": step}
            r = pd.DataFrame(r)
...
def many_logs2pandas(logdir_or_logfile, event_paths):
    all_logs = pd.DataFrame()
    for path in event_paths:
        path = str(path)
        log = tflog2pandas(logdir_or_logfile, path)
        if log is not None:
...
@click.option("--out", "-o", help="output file", default="./metrics.csv")
def main(logdir_or_logfile: str, write_pkl: bool, write_csv: bool, out: str):
    ...
    if os.path.isdir(logdir_or_logfile):
        ...
        event_paths = list(Path(logdir_or_logfile).rglob('event*'))
    ...
    if event_paths:
        ...
        all_logs = many_logs2pandas(logdir_or_logfile, event_paths)
        ...
        if write_csv:
            ...
            all_logs.to_csv(out, index=None)
        if write_pkl:
            print("saving to pickle file")
            all_logs.to_pickle(out)
    else:
